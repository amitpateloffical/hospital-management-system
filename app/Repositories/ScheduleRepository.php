...
class ScheduleRepository extends BaseRepository
{
    ...
    {
        $breakQuery = LunchBreak::where('doctor_id', $input['doctor_id']);
        ...
        if(isset($input['date'])){
            ...
            $data['doctorBreak'] = $breakQuery->where('date',$input['date'])->get();
            if($data['doctorBreak']->count() == 0){
                $data['doctorBreak'] = $breakQuery->whereNotNull('every_day')->get();
            }
        }else{
